...
class DCGAN(object):
  ...
  def build_model(self):
    ...
    with tf.device(self.gpu_allocator.discriminator_device()):
      self.d_loss_real = tf.math.reduce_variance(d_loss_real_input_tensor)
      self.d_loss_fake = tf.math.reduce_variance(d_loss_fake_input_tensor)

    with tf.device(self.gpu_allocator.generator_device()):
      self.g_loss = tf.math.reduce_variance(g_loss_input_tensor)
